...
def main():
    ...
    test_dir = os.path.join(this_dir, 'results_with_tricks')
    ...
    for lr_path, hr_path, idx_test in zip(lr_paths, hr_paths, range(len(lr_paths))):
        ...
        lr = imread(lr_path)
        hr = imread(hr_path)
        dark = lr  # low-light image
        zero_array = (np.all(dark != 0, axis=-1) * 1.0)
        zero_array = np.stack([zero_array] * 3, axis=-1)
        zero_map = 1 - zero_array
        zero_map = t(zero_map)
        # Pad image to be % 2
        h, w, c = lr.shape
        lq_orig = lr.copy()
        ...
        with torch.cuda.amp.autocast():
            sr_t = model.get_sr(lq=lr_t.cuda(), mask=zero_map.cuda(), heat=None)
