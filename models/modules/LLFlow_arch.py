...
class LLFlow(nn.Module):
    ...
    @autocast()
    def forward(self, gt=None, lr=None, mask=None, z=None, eps_std=None, reverse=False, epses=None, reverse_with_grad=False,
                lr_enc=None,
                add_gt_noise=True, step=None, y_label=None, align_condition_feature=False, get_color_map=False):
        if get_color_map:
        ...
        if not reverse:
            ...
            return self.normal_flow(gt, lr, mask, epses=epses, lr_enc=lr_enc, add_gt_noise=add_gt_noise, step=step,
            ...
            if reverse_with_grad:
                return self.reverse_flow(lr, z, mask, y_onehot=y_label, eps_std=eps_std, epses=epses, lr_enc=lr_enc,
    ...
    def normal_flow(self, gt, lr, mask, y_onehot=None, epses=None, lr_enc=None, add_gt_noise=True, step=None,
                    align_condition_feature=False):
        if self.opt['to_yuv']:
            gt = self.rgb2yuv(gt)
        if lr_enc is None and self.RRDB:
            lr_enc = self.rrdbPreprocessing(lr, mask)

        logdet = torch.zeros_like(gt[:, 0, 0, 0])
        pixels = thops.pixels(gt)
        xl_latent = lr_enc['color_map']
        z = gt
    ...
    def rrdbPreprocessing(self, lr, mask):
        rrdbResults = self.RRDB(lr, mask)
        concat = rrdbResults["cat_f"]
        if opt_get(self.opt, ['network_G', 'flow', 'stackRRDB', 'concat']) or False:
            keys = ['fea_up0', 'fea_up1', 'fea_up2']
            for k in keys:  #####k=['fea_up0', 'fea_up1', 'fea_up2']
                h = rrdbResults[k].shape[2]
                w = rrdbResults[k].shape[3]
                rrdbResults[k] = torch.cat([rrdbResults[k], F.interpolate(concat, (h, w), mode='bilinear')], dim=1)
    ...
    def reverse_flow(self, lr, z, mask, y_onehot, eps_std, epses=None, lr_enc=None, add_gt_noise=True):
