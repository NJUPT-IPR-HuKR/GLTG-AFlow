...
class LLFlowModel(BaseModel):
    ...
    def optimize_color_encoder(self, step):
        ...
        color_lr, color_gt = self.fake_H[(0, 0)], logdet = self.netG(lr=self.var_L, gt=self.real_H, mask=self.mask,
                                                                     get_color_map=True)
        losses = {}
    ...
    def optimize_parameters(self, step):
        ...
        if weight_fl > 0:
            if self.opt['optimize_all_z']:
                ...
                epses = None
            z, nll, y_logits = self.netG(gt=self.real_H, lr=self.var_L, mask=self.mask, reverse=False, epses=epses,
        ...
        if self.heats is not None:
            for heat in self.heats:
                for i in range(self.n_sample):
                    ...
                    with torch.no_grad():
                        self.fake_H[(heat, i)], logdet = self.netG(lr=self.var_L, mask=self.mask, z=z, eps_std=heat, reverse=True)
        else:
            z = self.get_z(0, seed=None, batch_size=self.var_L.shape[0], lr_shape=self.var_L.shape)
            with torch.no_grad():
                self.fake_H[(0, 0)], logdet = self.netG(lr=self.var_L, mask=self.mask, z=z.to(self.var_L.device), eps_std=0, reverse=True,
                                                        add_gt_noise=True)
        self.netG.train()
        return None
    ...
    def get_sr(self, lq, mask=None, heat=None, seed=None, z=None, epses=None):
        return self.get_sr_with_z(lq, mask, heat, seed, z, epses)[0]
    ...
    def get_sr_with_z(self, lq, mask=None, heat=None, seed=None, z=None, epses=None):
        self.netG.eval()
        if heat is None:
            heat = 0
        ...
        with torch.no_grad():
            sr, logdet = self.netG(lr=lq, mask=mask, z=z, eps_std=heat, reverse=True, epses=epses)
        self.netG.train()
        return sr, z
